...
public class UiMainMenu
{
    ...
    private void UpdateLogin(KeyboardState ks)
    {
        ...
        {
            if (_activeField == ActiveField.Username)
            {
    ...
    private void ProcessTextInput(KeyboardState ks)
    {
        foreach (var key in ks.GetPressedKeys())
        {
            ...
            {
                var character = GetCharFromKey(key, ks.IsKeyDown(Keys.LeftShift) || ks.IsKeyDown(Keys.RightShift));
                if (character != '\0')
                {
                    if (_activeField == ActiveField.Username)
                    {
                    ...
                    else if (_activeField == ActiveField.Password)
                    {
                        _password += character;
                    }
                }
                else if (key == Keys.Back &&
                         (_activeField == ActiveField.Username ? _username.Length > 0 : _password.Length > 0))
                {
                    if (_activeField == ActiveField.Username)
                    {
    ...
    public void Draw(SpriteBatch sb)
    {
        if (_background != null)
        {
            sb.Draw(_background, new Rectangle(0, 0, _graphicsDevice.Viewport.Width, _graphicsDevice.Viewport.Height),
                Color.White);
        }
    ...
    {
        if (msg.Op != Opcode.LoginResponse)
        {
            return;
        }

        if (string.IsNullOrWhiteSpace(msg.JsonPayload))
        {
            return;
        }
    ...
    {
        var yPos = _optionsStart.Y;

        // Username
        var userText = Loc.T("UI_Username") + _username + (_activeField == ActiveField.Username ? "_" : "");
        var userPos = new Vector2(_graphicsDevice.Viewport.Width / 2 - _optionFont.MeasureString(userText).X / 2, yPos);
        sb.DrawString(_optionFont, userText, userPos, Color.White);
        yPos += _optionSpacing;

        // Password
        var passText = Loc.T("UI_Password") + new string('*', _password.Length) +
                       (_activeField == ActiveField.Password ? "_" : "");
        var passPos = new Vector2(_graphicsDevice.Viewport.Width / 2 - _optionFont.MeasureString(passText).X / 2, yPos);
        sb.DrawString(_optionFont, passText, passPos, Color.White);
        yPos += _optionSpacing * 1.5f;

        // Login/Back Buttons
        for (var i = 0; i < _loginOptions.Count; i++)
        {
        ...
        {
            var statusPos = new Vector2(_graphicsDevice.Viewport.Width / 2,
                yPos + _loginOptions.Count * _optionSpacing);
            var statusOrigin = _optionFont.MeasureString(_loginStatusMessage) / 2;
            sb.DrawString(_optionFont, _loginStatusMessage, statusPos, _loginStatusColor, 0f, statusOrigin, 1f,
