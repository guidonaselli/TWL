...
namespace TWL.Tests
{
    public class ContentIntegrationTests
    {
        ...
        private List<T> LoadContent<T>(string filename)
        {
            ...
            if (!File.Exists(path)) throw new FileNotFoundException($"Could not find {filename} at {Path.GetFullPath(path)}");
        ...
        private string GetRepoRoot()
        {
            ...
            while (current != null)
            {
                if (Directory.Exists(Path.Combine(current.FullName, "TWL.Client"))) return current.FullName;
                current = current.Parent;
            }
            return "../../../../"; // Fallback
        }
