...
public class PipelineStressTest
{
    ...
    public async Task Pipeline_ShouldPopulateHistograms_UnderLoad()
    {
        ...
        try
        {
            ...
            // Assert
            Assert.True(snapshot.NetMessagesProcessed >= msgCount, $"Expected >= {msgCount} processed, got {snapshot.NetMessagesProcessed}");
            ...
            long totalVal = valHist.Bucket1ms + valHist.Bucket5ms + valHist.Bucket10ms + valHist.Bucket50ms + valHist.Bucket100ms + valHist.BucketOver100ms;
            long totalRes = resHist.Bucket1ms + resHist.Bucket5ms + resHist.Bucket10ms + resHist.Bucket50ms + resHist.Bucket100ms + resHist.BucketOver100ms;
