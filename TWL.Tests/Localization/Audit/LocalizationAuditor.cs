...
{
    private readonly Dictionary<string, string> _baseResourceValues = new();
        ...
        foreach (var file in sourceFiles)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                if (method == "TF")
                {
                    ...
                    var argCount = CountArgs(content, startIndex);
    ...
    private int CountArgs(string content, int startIndex)
    {
        ...
        for (; i < content.Length; i++)
        {
            ...
            else if (c == ')')
            {
                if (parensLevel == 0)
                {
                    return args; // End of Loc.TF call
                }
    ...
    private void ScanHardcodedStrings(AuditResults results)
    {
        ...
        foreach (var file in files)
        {
            ...
            if (_config.AllowedFolders.Any(ignored =>
                    file.Contains(Path.DirectorySeparatorChar + ignored + Path.DirectorySeparatorChar)))
            {
                continue;
            }

            if (_config.IgnoredFiles.Any(ignored => Path.GetFileName(file) == ignored))
            {
                continue;
            }
            ...
            for (var i = 0; i < lines.Length; i++)
            {
                ...
                foreach (Match match in matches)
                {
                    ...
                    if (val.Length < 2)
                    {
                        continue; // Skip single chars usually
                    }

                    if (val.Contains(".json") || val.Contains(".png") || val.Contains("/"))
                    {
                        continue; // likely paths
                    }

                    // Check allowlist literals
                    if (_config.AllowedHardcodedLiterals.Contains(val))
                    {
                        continue;
                    }
                    ...
                    if (Regex.IsMatch(line, $@"Loc\.TF?\(\s*""{escapedVal}"""))
                    {
                        continue;
                    }
        ...
        foreach (var key in allUsedKeys)
        {
            if (!validPrefixes.Any(p => key.StartsWith(p)))
            {
                results.Findings.Add(new AuditFinding
                {
                    ...
                    Location = key,
                    Message =
                        $"Key '{key}' does not follow naming convention (prefixes: {string.Join(", ", validPrefixes)})",
        ...
        foreach (var kvp in results.UsedKeys.TfArgCounts)
        {
            ...
            if (_baseResourceValues.TryGetValue(key, out var resourceValue))
            {
                ...
                foreach (Match m in matches)
                {
                    ...
                    {
                        if (idx > maxPlaceholderIndex)
                        {
                            maxPlaceholderIndex = idx;
                        }
                    }
                }
                ...
                foreach (var count in argCounts)
                {
                    if (count != expectedArgs)
                    {
                        results.Findings.Add(new AuditFinding
                        {
                            ...
                            Location = key,
                            Message =
                                $"Format mismatch for '{key}': Resource has {expectedArgs} placeholders, code supplies {count} arguments.",
