...
public static class MapValidator
{
    ...
    private static void ValidateCollisions(XElement layer)
    {
        ...
        {
            var collisionType = GetProperty(obj, "CollisionType");
            if (string.IsNullOrEmpty(collisionType))
            {
                throw new Exception(
                    $"Collision object (ID {obj.Attribute("id")?.Value}) missing 'CollisionType' property.");
            }

            if (!WorldConstants.ValidCollisionTypes.Contains(collisionType))
            {
                throw new Exception(
                    $"Collision object (ID {obj.Attribute("id")?.Value}) has invalid 'CollisionType': {collisionType}.");
            }
        }
    }
    ...
    private static void ValidateSpawns(XElement layer)
    {
        foreach (var obj in layer.Elements("object"))
        {
            var spawnType = GetProperty(obj, "SpawnType");
            ...
            {
                throw new Exception(
                    $"Spawn object (ID {obj.Attribute("id")?.Value}) has invalid 'SpawnType': {spawnType}.");
            }
    ...
    private static void ValidateTriggers(XElement layer)
    {
        ...
        foreach (var obj in layer.Elements("object"))
        {
            var triggerType = GetProperty(obj, "TriggerType");
            if (string.IsNullOrEmpty(triggerType))
            {
                throw new Exception(
                    $"Trigger object (ID {obj.Attribute("id")?.Value}) missing 'TriggerType' property.");
            }

            if (!WorldConstants.ValidTriggerTypes.Contains(triggerType))
            {
                throw new Exception(
                    $"Trigger object (ID {obj.Attribute("id")?.Value}) has invalid 'TriggerType': {triggerType}.");
            }
    ...
    private static void ValidatePropertyExists(XElement obj, string propertyName)
    {
        ...
        {
            throw new Exception(
                $"Object (ID {obj.Attribute("id")?.Value}) missing required property '{propertyName}'.");
        }
    }
    ...
    private static void ValidateMeta(string path)
    {
        ...
        try
        {
            using (var doc = JsonDocument.Parse(json))
            {
                ...
                if (!root.TryGetProperty("MapId", out _))
                {
                    throw new Exception("Meta missing 'MapId'");
                }

                if (!root.TryGetProperty("RegionId", out _))
                {
                    throw new Exception("Meta missing 'RegionId'");
                }
