# ROADMAP — The Wonderland (TWL: WLO-like Online JRPG)
> Documento único de priorización y secuenciación. Todo PR diario debe mapear a un ítem de este roadmap (o crear uno nuevo con criterio y no churn).
...
## 1) Definición de “listo” (DoD) por tipo de ítem
### 1.1 Feature (mecánica)
...
### 1.3 Contenido (skills/quests/items)
- Definición en JSON con schema y validación.
- Coverage mínima (si es skill pack: 3 ramas; si es questline: gating + rewards + flags).
...
## 4) Backlog ejecutable (IDs + orden)
> Regla: Jules toma el **primer ítem no completado** del dominio correspondiente al día.
...
### 4.4 SKILLS SYSTEM (P1)
- [ ] **SKL-001**: Schema JSON definitivo + validador (IDs únicos, referencias, constraints por tier).
- [ ] **SKL-002**: Mastery por uso + thresholds + persistencia + stage upgrades (1/2/3 con {6,12}).
- [ ] **SKL-003**: UnlockRules (level/stats/pre-skill mastery/quest flag).
...
- [ ] **SKL-010**: Counters avanzados (tempo disruption vs stability, etc.) con tradeoffs.

### 4.5 QUESTS SYSTEM (P1)
...
## 5) Rotación diaria recomendada (para Jules Tasks)
...
- **Día 4 (Skills)**: tomar próximo `SKL-*`.
- **Día 5 (Quests)**: tomar próximo `QST-*`.
- Repetir ciclo. Si un dominio está bloqueado, se registra en `reports/jules/blockers/YYYY-MM-DD.md` y se toma el siguiente ítem desbloqueante.

---

## 6) Convenciones de documentación (mínimo viable)
- `docs/architecture/`: contratos de servicios + diagramas simples.
- `docs/skills/`: schema, budgets por tier, packs por elemento, matriz de counters.
- `docs/quests/`: schema, patrones de questlines, ejemplos.
- `docs/economy/`: ledger/idempotencia/compound/market/mall.
- `docs/security/`: threat model mínimo + checklist de hardening.
- `docs/ops/`: métricas, health checks, recovery.

---

## 7) Regla anti-churn (muy importante)
Un PR se rechaza si:
...
- No se puede mapear a un ítem de este ROADMAP.

---

## 8) Cómo agregar nuevos ítems al ROADMAP
Formato:
