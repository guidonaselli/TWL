...
must_haves:
  ...
  key_links:
    - from: "TWL.Server/Simulation/Program.cs"
      ...
      pattern: "AddDbContext.*GameDbContext"
    - from: "GameDbContext"
      to: "PostgreSQL"
  ...
  <action>
    ...
    2. Update `TWL.Server/Simulation/Program.cs` DI registration:
       ...
       - BEFORE the existing `DbService` registration, add NpgsqlDataSource as singleton:
         ...
         ```
       - After NpgsqlDataSource, register GameDbContext:
         ```csharp
         svcs.AddDbContext<GameDbContext>(options =>
    ...
    IMPORTANT: `AddDbContext` registers as Scoped by default. For a game server that doesn't use HTTP request scoping, consider using `ServiceLifetime.Transient` or creating scopes manually in PlayerService. Register as:
    ```csharp
    svcs.AddDbContext<GameDbContext>(options =>
        options.UseNpgsql(dataSource),
        ServiceLifetime.Transient);
    ```
    This ensures each repository operation gets a fresh DbContext (no stale change tracker state between saves).
  </action>
  <verify>
