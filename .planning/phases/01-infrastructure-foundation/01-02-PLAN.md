...
<task type="auto">
  ...
  <action>
    ...
    IMPORTANT: The `GameDbContext` is registered as Transient (from Plan 01). For `DbPlayerRepository`, receive it via constructor injection. Since PlayerService is Singleton and DbPlayerRepository will be Singleton, you CANNOT inject a Transient DbContext directly. Instead:
       - Option A: Inject `IServiceProvider` and resolve `GameDbContext` per-operation using `CreateScope()`
       - Option B: Inject `IDbContextFactory<GameDbContext>` (requires `AddDbContextFactory` in DI)
       - **Use Option B** - cleaner pattern. Update Plan 01's DI registration note: use `AddDbContextFactory<GameDbContext>` instead of `AddDbContext<GameDbContext>` in Program.cs. The factory creates short-lived contexts per operation.

    Update DbPlayerRepository constructor to accept `IDbContextFactory<GameDbContext>`:
    ```csharp
...
<task type="auto">
  ...
  <action>
    1. Update `TWL.Server/Simulation/Program.cs`:
       ...
       - Change the EF Core registration from `AddDbContext` to `AddDbContextFactory`:
         ```csharp
